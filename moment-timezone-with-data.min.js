!(function (M, b) {
   ...
   var M = A.version.split("."),
      z = +M[0],
      p = +M[1];
   function n(M) {
      return 96 < M ? M - 87 : 64 < M ? M - 29 : M - 48;
   }
   function O(M) {
      var b = 0,
         z = M.split("."),
         ...
         c = 0,
         q = 1;
      for (45 === M.charCodeAt(0) && (q = -(b = 1)); b < p.length; b++)
         c = 60 * c + n(p.charCodeAt(b));
      for (b = 0; b < O.length; b++) (A /= 60), (c += n(O.charCodeAt(b)) * A);
      return c * q;
   }
   function R(M) {
      for (var b = 0; b < M.length; b++) M[b] = O(M[b]);
   }
   function X(M, b) {
      var z,
         p = [];
   ...
   function N(M, b) {
      for (var z, p; (p = 6e4 * (((b.at - M.at) / 12e4) | 0)); )
         (z = new f(new Date(M.at + p))).offset === M.offset
            ? (M = z)
            : (b = z);
      return M;
   }
   ...
   function u(M, b) {
      var z, p;
      for (R(b), z = 0; z < b.length; z++)
         (p = b[z]), (d[p] = d[p] || {}), (d[p][M] = !0);
   }
   function e() {
      try {
      ...
      var z,
         ...
         A = (function () {
            var M,
               ...
               O = new f(new Date(p, 0, 1)),
               A = [O];
            for (z = 1; z < 48; z++)
               (b = new f(new Date(p, z, 1))).offset !== O.offset &&
                  ((M = N(O, b)),
                  A.push(M),
                  A.push(new f(new Date(M.at + 6e4)))),
                  (O = b);
            for (z = 0; z < 4; z++)
               A.push(new f(new Date(p + z, 0, 1))),
                  A.push(new f(new Date(p + z, 6, 1)));
            return A;
         })(),
         c = A.length,
         q = (function (M) {
            var b,
               ...
               c = [];
            for (b = 0; b < O; b++)
               for (z in (p = d[M[b].offset] || {}))
                  p.hasOwnProperty(z) && (A[z] = !0);
            for (b in A) A.hasOwnProperty(b) && c.push(W[b]);
            return c;
         })(A),
         o = [];
      for (p = 0; p < q.length; p++) {
         for (z = new i(t(q[p]), c), O = 0; O < c; O++) z.scoreOffsetAt(A[O]);
         o.push(z);
      }
      return o.sort(l), 0 < o.length ? o[0].zone.name : void 0;
   }
   function r(M) {
   ...
   function t(M, b) {
      ...
      return p instanceof B
         ...
         ? ((p = new B(p)), (c[M] = p))
         : q[M] && b !== t && (z = t(q[M], t))
         ? ((p = c[M] = new B())._set(z), (p.name = W[M]), p)
         : null;
   }
   function s(M) {
      var b, z, p, O;
   ...
   function S(M) {
      var b = Array.prototype.slice.call(arguments, 0, -1),
         ...
         p = t(z),
         O = A.utc.apply(null, b);
      return (
         p && !A.isMoment(M) && E(O) && O.add(p.parse(O), "minutes"), O.tz(z), O
      );
   }
   (z < 2 || (2 == z && p < 6)) &&
      m(
         ...
         parse: function (M) {
            ...
            for (O = 0; O < o; O++)
               if (
                  ...
                  (z = c[O + 1]),
                  (p = c[O ? O - 1 : O]),
                  b < z && S.moveAmbiguousForward
                     ? (b = z)
                     : p < b && S.moveInvalidForward && (b = p),
                  A < q[O] - 6e4 * b)
               )
                  return c[O];
            return c[o];
      ...
      (S.names = function () {
         var M,
            b = [];
         for (M in W)
            W.hasOwnProperty(M) && (c[M] || c[q[M]]) && W[M] && b.push(W[M]);
         return b.sort();
      }),
      (S.Zone = B),
      (S.unpack = a),
      ...
      (S.zonesForCountry = function (M, b) {
         if (
            !(M = (function (M) {
               return (M = M.toUpperCase()), o[M] || null;
            })(M))
         )
            return null;
         var z = M.zones.sort();
         return b
            ? z.map(function (M) {
                 return { name: M, offset: t(M).utcOffset(new Date()) };
              })
            : z;
      });
   var g,
   ...
   (A.tz = S),
         ...
         if (
            (void 0 === M._z &&
               (p &&
                  E(M) &&
                  !M._isUTC &&
                  ((M._d = A.utc(M._a)._d), M.utc().add(p.parse(M), "minutes")),
               (M._z = p)),
            M._z)
         )
            if (
      ...
      (A.tz.setDefault = function (M) {
         return (
            ...
            (A.defaultZone = M ? t(M) : null),
            A
         );
      });
   var k = A.momentProperties;
   return (
